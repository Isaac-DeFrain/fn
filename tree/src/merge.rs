...
fn merge(base_tree: &mut Tree<i32>, incoming_tree: &mut Tree<i32>, junction_node_id: &NodeId) {
    ...
    let new_node_id = base_tree
        .insert(Node::new(*node_data), UnderNode(junction_node_id))
        .unwrap();
    merge_id_map.insert(incoming_root_id, new_node_id);
    let traversal_ids = incoming_tree
...
pub fn merge_example() {
    ...
    let _node_id3 = base_tree
        .insert(Node::new(3), UnderNode(&node_id1))
        .unwrap();
    let _node_id4 = base_tree
        .insert(Node::new(-1), UnderNode(&node_id1))
        .unwrap();
    let _node_id5 = base_tree
        .insert(Node::new(-2), UnderNode(&node_id2))
        .unwrap();

    // Incoming tree
    let mut incoming_tree: Tree<i32> = Tree::new();
    let incoming_root_id = incoming_tree.insert(Node::new(4), AsRoot).unwrap();
    let incoming_node_id1 = incoming_tree
        .insert(Node::new(5), UnderNode(&incoming_root_id))
        .unwrap();
    let incoming_node_id2 = incoming_tree
        .insert(Node::new(6), UnderNode(&incoming_root_id))
        .unwrap();
    let _incoming_node_id3 = incoming_tree
        .insert(Node::new(7), UnderNode(&incoming_node_id1))
        .unwrap();
    let _incoming_node_id4 = incoming_tree
        .insert(Node::new(8), UnderNode(&incoming_node_id2))
        .unwrap();
    let _incoming_node_id5 = incoming_tree
        .insert(Node::new(9), UnderNode(&incoming_node_id2))
        .unwrap();
    ...
    let parent_id = base_tree.get(&node_id1).unwrap().parent().unwrap();
