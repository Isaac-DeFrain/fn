...
impl Vec<Stmt> {
    fn remove_node(&mut self, nid: NodeId) -> Vec<Stmt> {
        ...
        for s in self.iter() {
            match s {
                ...
                // remove edges connecting to `nid`
                Stmt::Edge(Edge { ty: EdgeTy::Pair(Vertex::N(x), Vertex::N(y)), ..}) => {
                    if x != nid && y != nid {
                        stmts.push(s);
                    }
                }
                // remove `nid` from chains
                Stmt::Edge(Edge { ty: EdgeTy::Chain(vs), attributes }) => {
                    let mut new_vs : Vec<Vertex> = Vec::new();
                    for v in vs {
                        match v {
                            Vertex::N(id) => {
                                if x != nid {
                            ...
                            Vertex::S(s) => {
                                let new_s = Subgraph { id: s.id, stmts: s.stmts.remove_node(nid) };
                                new_vs.push(Vertex::S(new_v));
                            }
                        }
                    }
                    // TODO remove attrbiutes
                    stmts.push(Stmt::Edge(Edge { ty: EdgeTy::Chain(new_vs), attributes }));
                }
                // TODO remove other edges
                // TODO remove from attributes
                // TODO remove from gattributes
            }
        }
        stmts
    }
    ...
    pub fn remove_node(&mut self, n: NodeId) {
        match self {
            Graph::DiGraph { id, strict, stmts } => {
                Graph::DiGraph { id, strict, stmts: stmts.remove_node(n) }
            }
            Graph::Graph { id, strict, stmts } => {
                Graph::Graph { id, strict, stmts: stmts.remove_node(n) }
            }
        }
    }
}
